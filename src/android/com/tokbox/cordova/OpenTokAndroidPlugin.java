...
public class OpenTokAndroidPlugin extends CordovaPlugin implements
		...
		Session.SignalListener, PublisherKit.PublisherListener,
	...
	public boolean publishCalled; // we need this because creating publisher
									// before sessionConnected = crash
	...
	public class RunnableUpdateViews implements Runnable {
		...
		public class CustomComparator implements
				Comparator<RunnableUpdateViews> {
			@Override
			public int compare(RunnableUpdateViews object1,
					RunnableUpdateViews object2) {
		...
		public void updateZIndices() {
			...
			for (Map.Entry<String, RunnableSubscriber> entry : subscriberCollection
					.entrySet()) {
			...
			for (RunnableUpdateViews viewContainer : allStreamViews) {
				ViewGroup parent = (ViewGroup) cordova.getActivity()
		...
		public void run() {
			try {
				Log.i(TAG,
						"updating view in ui runnable" + mProperty.toString());
				Log.i(TAG, "updating view in ui runnable " + mView.toString());

				float widthRatio, heightRatio;

				// Ratios are index 6 & 7 on TB.updateViews, 8 & 9 on subscribe
				// event, and 9 & 10 on TB.initPublisher
				int ratioIndex;
				...
				Log.i(TAG,
						"error when trying to retrieve properties while resizing properties");
			}
		}
	}
	...
	public class RunnablePublisher extends RunnableUpdateViews implements
		...
		public void destroyPublisher() {
			ViewGroup parent = (ViewGroup) cordova.getActivity().findViewById(
		...
		public void run() {
			...
			if (mPublisher == null) {
				ViewGroup frame = (ViewGroup) cordova.getActivity()
						.findViewById(android.R.id.content);
				...
				mPublisher = new Publisher(cordova.getActivity()
						.getApplicationContext(), publisherName);
				...
				try {
					...
					Log.i(TAG,
							"error when trying to retrieve publish audio/video property");
				}
	...
	public class RunnableSubscriber extends RunnableUpdateViews implements
		...
		// property contains: [stream.streamId, position.top, position.left,
		// width, height, subscribeToVideo, zIndex] )
		...
		public void removeStreamView() {
			ViewGroup frame = (ViewGroup) cordova.getActivity().findViewById(
					android.R.id.content);
		...
		public void onError(SubscriberKit arg0, OpentokError arg1) {
			...
			Log.e(TAG, "subscriber exception: " + arg1.getMessage()
					+ ", stream id: " + arg0.getStream().getStreamId());
		}
	...
	public void initialize(CordovaInterface cordova, CordovaWebView webView) {
		...
		// By default, get a pointer to mainView and add mainView to the
		// viewList as it always exists (hold cordova's view)
	...
	public boolean execute(String action, JSONArray args,
		...
		if (action.equals("initPublisher")) {
			...
			mSession = new Session(this.cordova.getActivity()
					.getApplicationContext(), args.getString(0),
					args.getString(1));
			...
			if (args.getString(0).equals("TBPublisher") && myPublisher != null
				...
				RunnableSubscriber runsub = subscriberCollection.get(args
						.getString(0));
				if (runsub != null) {
	...
	public void onConnected(Session arg0) {
		Log.i(TAG,
				"session connected, triggering sessionConnected Event. My Cid is: "
						+ mSession.getConnection().getConnectionId());
		sessionConnected = true;

		connectionCollection.put(mSession.getConnection().getConnectionId(),
				mSession.getConnection());

		JSONObject data = new JSONObject();
		try {
			data.put("status", "connected");
			JSONObject connection = createDataFromConnection(mSession
					.getConnection());
	...
	public void onDisconnected(Session arg0) {
		...
		// delete all data and prevent updateviews from drawing non existent
		// things
	...
	public void onStreamDropped(Session arg0, Stream arg1) {
		...
		streamCollection.remove(arg1.getStreamId());
		RunnableSubscriber subscriber = subscriberCollection.get(arg1
				.getStreamId());
		if (subscriber != null) {
	...
	// signalListener
	public void onSignalReceived(Session arg0, String arg1, String arg2,
			Connection arg3) {
		JSONObject data = new JSONObject();
	...
	public void triggerJSEvent(String event, String type, JSONObject data) {
		...
		PluginResult myResult = new PluginResult(PluginResult.Status.OK,
				message);
